...
public class J_EAProduction extends zero_engine.J_EA implements Serializable {
	 ...
	 */
	public J_EAProduction(Agent parentAgent, OL_EnergyAssetType type, String name, double capacityElectric_kW,
			double capacityHeat_kW, double yearlyProductionMethane_kWh, double yearlyProductionHydrogen_kWh,
			double timestep_h, double outputTemperature_degC, J_ProfilePointer profile) {
	...
	public void setCapacityElectric_kW(double capacityElectric_kW) {
		// Calculate the difference with the set and the previous capacity to update
		// totals in GC, GN and EnergyModel
		if (energyCarrier == OL_EnergyCarriers.ELECTRICITY) {
			...
			if (this.energyAssetType == OL_EnergyAssetType.WINDMILL && this.parentAgent instanceof GridConnection) {
				...
				if (((GridConnection) this.parentAgent).l_parentNodeElectric.getConnectedAgent() != null) {
					((GridConnection) this.parentAgent).l_parentNodeElectric.getConnectedAgent()
							.f_updateTotalInstalledProductionAssets(OL_EnergyAssetType.WINDMILL, difference_kW, true);
				}
				((GridConnection) this.parentAgent).energyModel.v_totalInstalledWindPower_kW += difference_kW;
					...
					&& this.parentAgent instanceof GridConnection) {
				((GridConnection) this.parentAgent).v_totalInstalledPVPower_kW += difference_kW;
				if (((GridConnection) this.parentAgent).l_parentNodeElectric.getConnectedAgent() != null) {
					((GridConnection) this.parentAgent).l_parentNodeElectric.getConnectedAgent()
							.f_updateTotalInstalledProductionAssets(OL_EnergyAssetType.PHOTOVOLTAIC, difference_kW,
									true);
				}
	...
	// public Pair<J_FlowsMap, Double> curtailElectricityProduction(double
	// curtailmentSetpoint_kW) {
	public void curtailElectricityProduction(double curtailmentSetpoint_kW) { // This variable is called
																				// curtailmentSetpoint, but maybe its
																				// better to call it curtailment amount?
																				// it represents the amount of
																				// production we need to curtail, not
																				// the amount we want to produce.
		...
		// double[] arr = {-curtailmentPower_kW, 0, 0, 0, 0, 0, 0, 0, 0,
		// curtailmentPower_kW};
		...
		// traceln("Electricity production of asset %s curtailed by %s kW!", this,
		// curtailmentPower_kW);
		if (parentAgent instanceof GridConnection) {
