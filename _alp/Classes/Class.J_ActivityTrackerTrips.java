   ...
   public void manageActivities(J_TimeVariables timeVariables, I_ChargePointRegistration chargePointRegistration) {
    ...
    public void prepareNextActivity(double time_min, I_ChargePointRegistration chargePointRegistration) {
		...
		if (vehicle instanceof J_EAEV ev) {
			...
			if (v_idleTimeToNextTrip_min > 0 && (energyNeedForNextTrip_kWh-ev.getCurrentSOC_kWh())> v_idleTimeToNextTrip_min/60 * ev.getVehicleChargingCapacity_kW()) {
			...
			/*if ( (v_energyNeedForNextTrip_kWh - EV.getCurrentStateOfCharge() * EV.getStorageCapacity_kWh()) / (v_idleTimeToNextTrip_min/60) > EV.getVehicleChargingCapacity_kW() ) {
